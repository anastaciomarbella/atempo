...
const AgendaSemanal = () => {
  ...
  const fetchCitas = async () => {
    ...
    try {
      ...
      // Filtrar por usuario usando String
  ...
  const citasPorDia = semanaFechas.map(fecha=>{
    return citas.filter(c=>{
      ...
      return citaDate.getFullYear()===fecha.getFullYear() &&
             citaDate.getMonth()===fecha.getMonth() &&
             citaDate.getDate()===fecha.getDate();
    });
  });

  // 🔹 Calcular posición y altura de cita
        ...
        {HORAS_DIA.map(hora=>(
          <React.Fragment key={hora}>
            {semanaFechas.map((fecha,idx)=>{
              const citasHora = citasPorDia[idx].filter(c=>{
                const [hInicio] = c.hora_inicio?.split(":").map(Number) || [];
                return hInicio === hora;
              });
              return (
                <div key={idx+"-"+hora} className="hour-cell" style={{position:"relative"}}>
                  {citasHora.map(cita=>(
                    <div key={cita.id_cita} className="event-card"
